...
const ClientRpcEvents: {
  [eventName: string]: (...args: any[]) => Promise<any> | any;
} = {};
...
async function handleFrontendSound(
  audioName: string,
  audioRef: string,
  audioBank = ''
) {
...
export function useWebview(path = 'http://assets/webview/index.html') {
   ...
   * @param {PageType} type
   */
  function show(pageName: PageNames, type: PageType, escapeToClose = false) {
    webview.emit(Events.view.show, pageName, type);

    if (type === 'page') {
      focus();
      isPageOpen = true;
      openPages.push(pageName);
      if (escapeToClose) {
        escapeToClosePage = pageName;
      }

      alt.emitServer(Events.view.onPageOpen, pageName);
      return;
    }

    if (type === 'overlay') {
      const idx = openOverlays.findIndex((x) => x === pageName);
      if (idx >= 0) {
        return;
      }

      openOverlays.push(pageName);
      return;
    }

    if (type === 'persistent') {
      const idx = openPeristentPages.findIndex((x) => x === pageName);
      if (idx >= 0) {
        return;
      }

      openPeristentPages.push(pageName);
      return;
    }
  }
   ...
   * @param {PageNames} pageName
   */
  function hide(pageName: PageNames) {
    webview.emit(Events.view.hide, pageName);
    unfocus();

    if (escapeToClosePage === pageName) {
      escapeToClosePage = undefined;
    }

    if (onCloseEvents[pageName]) {
      for (let cb of onCloseEvents[pageName]) {
        cb();
      }
    }

    // Only remove 'page' types
    const index = openPages.findIndex((page) => page === pageName);
    if (index > -1) {
      isPageOpen = false;
      openPages.splice(index, 1);
    }

    alt.emitServer(Events.view.onPageClose, pageName);
  }

  /**
   * Hide all pages specified in this function
   *
   * @param {PageNames[]} pageNames
   */
  function hideAll(pageNames: PageNames[]) {
    webview.emit(Events.view.hideAll, pageNames);
  }
   ...
   * @returns {boolean}
   */
  function isSpecificPageOpen(pageName: PageNames): boolean {
    return openPages.findIndex((page) => page === pageName) > -1;
  }

  function isOverlayOpen(pageName: PageNames): boolean {
    return openOverlays.findIndex((page) => page === pageName) > -1;
  }

  function isPersistentPageOpen(pageName: PageNames) {
    return openPeristentPages.findIndex((page) => page === pageName) > -1;
  }
  ...
  function onClose(pageName: PageNames, callback: Function) {
    if (!onCloseEvents[pageName as string]) {
      onCloseEvents[pageName as string] = [];
    }
